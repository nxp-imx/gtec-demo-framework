...
layout(location = 0) in vec3 v_LightVec;
layout(location = 1) in vec3 v_HalfVec;
...
void main()
{
  ...
  if (intensity > 0.0)
  {
    ...
    spec = texture(TextureSpecular, v_TexCoord) * pow(intSpec, g_ubo.MatShininess);
  }

  o_fragColor = diffuse * max(intensity + spec, g_ubo.MatAmbient);
}
