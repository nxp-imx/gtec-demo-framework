...
void main()
{
  ...
  if (intensity > 0.0)
  {
    ...
    spec = g_ubo.MatSpecular * (pow(intSpec, g_ubo.MatShininess * col.w) * vec4(col.xyz, 1.0));
  }

  o_fragColor = texture(Texture0, v_TexCoord) * ((v_Color * intensity) + spec + g_ubo.MatAmbient);
}
