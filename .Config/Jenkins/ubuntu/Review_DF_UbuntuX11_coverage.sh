...
#FslBuild.py -t sdk -vv --BuildTime --Variants [config=Coverage] --UseFeatures $FSL_FEATURES -c install

FslBuild.py -t sdk --BuildTime --Variants [config=Coverage] --UseFeatures $FSL_FEATURES --RequireFeature [GoogleUnitTest]

# setup a baseline
lcov --no-external --capture --initial --directory . --output-file coverage_base.info

FslBuild.py -t sdk --BuildTime --Variants [config=Coverage] --UseFeatures $FSL_FEATURES --RequireFeature [GoogleUnitTest] --ForAllExe "(EXE) --gtest_output=xml:""$FSL_TEST_REPORTS/(PACKAGE_NAME).xml"""
